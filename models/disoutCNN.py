...
from disout import FCDisout, LinearScheduler
...
class CnnDisout(nn.Module):
    ...
    def __init__(self, num_class=10, dist_prob=0.5, alpha=5, nr_steps=5e3):
        ...
        self.fc = nn.Sequential(
            ...
            # nn.BatchNorm1d(2048),
            nn.ReLU(),
            LinearScheduler(FCDisout(dist_prob=dist_prob, alpha=alpha),
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            for idb in range(num_module):
                if isinstance(modulelist[idb], FCDisout):
                    dp_loc.append(idb)
